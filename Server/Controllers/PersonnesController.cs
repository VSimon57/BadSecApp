    ...
    public class PersonnesController : Controller
    {
        ...
        public IActionResult CreationPersonne([FromBody] Personne personne)
        {
            try
            {
                using (var conn = new SqliteConnection("Data Source=test.db"))
                {
                    ...
                    commande.CommandText = "INSERT INTO PERSONNES (nom, prenom, age) VALUES ('" + personne.Nom + "', '" + personne.Prenom + "', " + personne.Age.ToString() + ")"; // SECU Faille injection SQL
        ...
        public Tuple<List<Personne>, string> GetAll([FromQuery] string IndicationNom)
        {
            ...
            try
            {
                using (var conn = new SqliteConnection("Data Source=test.db"))
                {
                    // SECU Binder les paramètres pour éviter les injections 
                    ...
                    // SECU Si on retourne une liste de personnes on retourne tout les urls des personnes
        ...
        public ContentResult GenererFiche([FromQuery] string nom)
        {
            ...
            using (var conn = new SqliteConnection("Data Source=test.db"))
            {
                ...
                using (var reader = commande.ExecuteReader())
                {
                    ...
                    {
                        // SECU Si nom =  script (alerte js) on peut déclencher un script. 
                ...
                {
                    if (reader.Read())
                        sb.Append("<img src=\"").Append(reader.GetString(0)).AppendLine("\"/>"); // SECU faille XSS rémanente.
                }
            }
